...
```json
{
  ...
  "message": "string",
  "status": "active",
  "next_step": "introduce_sage | confirm_name | linkedin_connect | explain_objectives | resume_prompt | construct_experience_summary | define_goals | classify_user_type | introduce_app_features | done",
  "current_step": "same_as_next_step_or_null",
  "completed_steps": ["introduce_sage"],
  "pending_steps": ["confirm_name", "construct_experience_summary"],
  "progress_percent": 12,
  "ui_actions": null,
  "step_id": null,
  "ui_action_progress": {
...
- **`ui_action_progress`:** shows completion state of emitted UI actions. Final onboarding completion now requires mandatory steps + all emitted UI actions acknowledged as completed by client.
...
Migration note:
- `v1` onboarding endpoints remain for compatibility during rollout.
- `v2` is the breaking-contract path for all server-led flows.
...
```json
{
  ...
  "ui_action_progress": {
    ...
    "completed_actions": 1,
    "all_completed": false,
    "pending_targets": ["profile.resume.upload_cta"]
  }
}
```
...
```json
{
  ...
  "status": "active | completed",
  "next_step": "introduce_sage | confirm_name | linkedin_connect | explain_objectives | resume_prompt | construct_experience_summary | define_goals | classify_user_type | introduce_app_features | done",
  "current_step": "introduce_sage | confirm_name | linkedin_connect | explain_objectives | resume_prompt | construct_experience_summary | define_goals | classify_user_type | introduce_app_features | done",
  "completed_steps": ["string", "..."],
  "pending_steps": ["string", "..."],
  "progress_percent": 0,
  "ui_actions": null,
...
### List active onboarding conversations

Fetch all active onboarding conversations for current user so the client can let user resume one.

```http
GET /api/v1/onboarding/active-conversations
Authorization: Bearer <token>
```
  ...
  "conversations": [
    {
      "conversation_id": "uuid",
      "status": "active",
      "next_step": "define_goals",
      "current_step": "define_goals",
      "completed_steps": ["introduce_sage", "confirm_name"],
      "pending_steps": ["define_goals", "classify_user_type", "introduce_app_features"],
      "progress_percent": 40,
      "last_agent_message": "Tell me your top two goals.",
      "started_at": "ISO8601",
      "ui_actions": null,
      "step_id": null,
      "ui_action_progress": {
...
### Acknowledge onboarding UI action completion

```http
POST /api/v1/onboarding/{conversation_id}/ui-actions/complete
Content-Type: application/json
Authorization: Bearer <token>
```
...
```json
{
  ...
  "step_id": "linkedin_suggest_connect",
  "completed": true,
  "metadata": { "source": "client" }
}
```

**Response:** same shape as onboarding status payload with updated `ui_action_progress`.
