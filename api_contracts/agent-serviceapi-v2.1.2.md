...
**Onboarding start is idempotent:** `POST /api/v2/flows/start` with `flow_type: "ONBOARDING"` may be called without storing `flow_instance.id`. The server returns the **same** onboarding as **`GET`** would: **active** flows are returned as-is (resume); **completed** or **abandoned** flows are returned with **`start_message`** set (`"Onboarding is already completed."` or `"Onboarding was abandoned or expired."`). A **new** onboarding row is created only when the user has **no** prior onboarding `flow_instance` rows.
...
**`start_message` (only on `POST .../start`, and only for `flow_type: "ONBOARDING"`):** `null` when a new flow was created or an **active** flow is returned. Non-null when the latest onboarding for this user is **completed** or **abandoned** (see §7).

**Errors (`POST .../start`):** `400` if `flow_type` is invalid.
...
- **`steps[].title`:** Human-readable label from the server’s registered flow definition for that `step_key` (`null` if the flow type is unknown or the step no longer exists in the definition).
...
`start_message` is present only on responses from **`POST .../start`** when `flow_type` is **`ONBOARDING`** (see above).
