...
**Base URL:** `{origin}/api/v1` for production or `http://localhost:8000/api/v1` for development
**Version:** 0.1.0
...
- **Validation:** Agent Service verifies the JWT using the shared JWT_SECRET
Validate:
signature using JWT_SECRET
aud (audience)
iss (issuer)
exp (expiry)
- **User identification:** Extract user ID from configured claim (usually sub) and use it for resource scoping.
...
```json
{
  ...
  "status": "active",
  "next_step": "expertise | goals | references"
}
```
