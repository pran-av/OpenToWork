...
export async function POST(
  ...
  try {
    ...
    const { ok, status, data } = await agentRequest({
      accessToken: token,
      method: "POST",
      path: `/flows/${flowInstanceId}/ui-actions/ack`,
      body: payload,
      apiPrefix: "/api/v2",
    });
    ...
    console.error("Error acknowledging v2 flow ui action:", error);
    return noStoreJsonResponse({ error: "Failed to acknowledge ui action" }, 500);
  }
}
