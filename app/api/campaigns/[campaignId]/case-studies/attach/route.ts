...
import {
  attachExperienceCaseStudyToCampaign,
  detachExperienceCaseStudyFromCampaign,
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    await detachExperienceCaseStudyFromCampaign({
      campaign_id: campaignId,
      case_id: caseId,
    });
    ...
    const message = error instanceof Error ? error.message : "Failed to detach case study";
    return NextResponse.json(
      { error: message },
      { status: 400 }
    );
